...
impl CairoRunner {
    ...
    fn initialize_state(
        ...
        if let Some(prog_base) = self.program_base.clone() {
            let initial_pc =
                Relocatable::from((prog_base.segment_index(), prog_base.offset() + entrypoint));
    ...
    fn initialize_function_entrypoint(
        ...
        if let Some(base) = &self.execution_base {
            self.initial_fp = Some(Relocatable::from((
                base.segment_index(),
    ...
    fn relocate_trace(&mut self, relocation_table: &Vec<usize>) -> Result<(), TraceError> {
        ...
        for entry in trace {
            relocated_trace.push(RelocatedTraceEntry {
                pc: relocate_trace_register(&Relocatable::from(entry.pc), relocation_table)?,
                ap: relocate_trace_register(&Relocatable::from(entry.ap), relocation_table)?,
                fp: relocate_trace_register(&Relocatable::from(entry.fp), relocation_table)?,
            })
        }
...
mod tests {
    ...
    fn initialize_segments_with_base() {
        ...
        let program_base = Some(relocatable!(5, 9));
        cairo_runner.vm.segments.num_segments = 6;
        cairo_runner.initialize_segments(program_base);
        assert_eq!(cairo_runner.program_base, Some(relocatable!(5, 9)),);
        assert_eq!(cairo_runner.execution_base, Some(relocatable!(6, 0)),);
    ...
    fn initialize_state_empty_data_and_stack() {
        ...
        assert_eq!(cairo_runner.initial_pc, Some(relocatable!(1, 1)),);
    }
    ...
    fn initialize_state_some_data_empty_stack() {
        ...
        cairo_runner.program_base = Some(Relocatable::from((1, 0)));
        cairo_runner.execution_base = Some(relocatable!(2, 0));
        let stack = Vec::new();
        cairo_runner.initialize_state(1, stack).unwrap();
        assert_eq!(
    ...
    */
    fn initialize_and_run_function_call() {
        ...
        assert_eq!(
            trace[0],
            TraceEntry {
                pc: Relocatable::from((0, 3)).into(),
                ap: Relocatable::from((1, 2)).into(),
                fp: Relocatable::from((1, 2)).into(),
            }
        );
        assert_eq!(
            trace[1],
            TraceEntry {
                pc: Relocatable::from((0, 5)).into(),
                ap: Relocatable::from((1, 3)).into(),
                fp: Relocatable::from((1, 2)).into(),
            }
        );
        assert_eq!(
            trace[2],
            TraceEntry {
                pc: Relocatable::from((0, 0)).into(),
                ap: Relocatable::from((1, 5)).into(),
                fp: Relocatable::from((1, 5)).into(),
            }
        );
        assert_eq!(
            trace[3],
            TraceEntry {
                pc: Relocatable::from((0, 2)).into(),
                ap: Relocatable::from((1, 6)).into(),
                fp: Relocatable::from((1, 5)).into(),
            }
        );
        assert_eq!(
    ...
    */
    fn initialize_and_run_range_check_builtin() {
        ...
        assert_eq!(
            trace[0],
            TraceEntry {
                pc: Relocatable::from((0, 8)).into(),
                ap: Relocatable::from((1, 3)).into(),
                fp: Relocatable::from((1, 3)).into(),
            }
        );
        assert_eq!(
            trace[1],
            TraceEntry {
                pc: Relocatable::from((0, 9)).into(),
                ap: Relocatable::from((1, 4)).into(),
                fp: Relocatable::from((1, 3)).into(),
            }
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            TraceEntry {
                pc: Relocatable::from((0, 0)).into(),
                ap: Relocatable::from((1, 7)).into(),
                fp: Relocatable::from((1, 7)).into(),
            }
        );
        assert_eq!(
            trace[4],
            TraceEntry {
                pc: Relocatable::from((0, 1)).into(),
                ap: Relocatable::from((1, 7)).into(),
                fp: Relocatable::from((1, 7)).into(),
            }
        );
        ...
        assert_eq!(
            trace[6],
            TraceEntry {
                pc: Relocatable::from((0, 4)).into(),
                ap: Relocatable::from((1, 9)).into(),
                fp: Relocatable::from((1, 7)).into(),
            }
        );
        assert_eq!(
    ...
    */
    fn initialize_and_run_output_builtin() {
        ...
        assert_eq!(
            ...
            TraceEntry {
                pc: Relocatable::from((0, 0)).into(),
                ap: Relocatable::from((1, 11)).into(),
                fp: Relocatable::from((1, 11)).into(),
            }
        );
        assert_eq!(
            trace[9],
    ...
    fn initialize_and_run_output_range_check_builtin() {
        ...
        assert_eq!(
            ...
            TraceEntry {
                pc: Relocatable::from((0, 13)).into(),
                ap: Relocatable::from((1, 4)).into(),
                fp: Relocatable::from((1, 4)).into(),
            }
        );
        assert_eq!(
            trace[1],
            TraceEntry {
                pc: Relocatable::from((0, 14)).into(),
                ap: Relocatable::from((1, 5)).into(),
                fp: Relocatable::from((1, 4)).into(),
            }
        );
        ...
        assert_eq!(
            trace[11],
            TraceEntry {
                pc: Relocatable::from((0, 19)).into(),
                ap: Relocatable::from((1, 13)).into(),
                fp: Relocatable::from((1, 4)).into(),
            }
        );
        assert_eq!(
            trace[12],
            TraceEntry {
                pc: Relocatable::from((0, 20)).into(),
                ap: Relocatable::from((1, 14)).into(),
                fp: Relocatable::from((1, 4)).into(),
            }
        );
        assert_eq!(
            ...
            TraceEntry {
                pc: Relocatable::from((0, 0)).into(),
                ap: Relocatable::from((1, 16)).into(),
                fp: Relocatable::from((1, 16)).into(),
            }
        );
        assert_eq!(
            trace[14],
        ...
        assert_eq!(
            trace[17],
            TraceEntry {
                pc: Relocatable::from((0, 23)).into(),
                ap: Relocatable::from((1, 18)).into(),
                fp: Relocatable::from((1, 4)).into(),
            }
        );
        //Check the range_check builtin segment
